...
const search = () => {
  if(title == ''){
    if(userType == 1){
      router.push({ path: '/books', query: { title: search_input.value } })    
    }else{
      router.push({ path: '/admin/books', query: { title: search_input.value } }) 
    }
  }else{
    if(userType == 1){
      router.push({ path: '/searchTransit', query: { title: search_input.value } })    
    }else{
      router.push({ path: '/admin/searchTransit', query: { title: search_input.value } }) 
    }    
  }
