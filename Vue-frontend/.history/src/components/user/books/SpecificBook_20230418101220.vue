...
import { TimeOutline, BookmarksOutline, PencilOutline } from '@vicons/ionicons5'
...
onBeforeMount(() => {
  ...
  api_isCommented(id).then(res=>{
    operationState.value.isRated = res.data.data
  })
...
<template>
  <div>
    ...
    <div class="container">
      ...
      <div class="operation">
        <n-button secondary strong :render-icon="renderIcon1" class="operation-btn" color="rgb(2, 108, 2)" v-if="!operationState.isWanted">
          想读
        </n-button>

        <n-button disabled secondary strong :render-icon="renderIcon1" class="operation-btn" color="rgb(2, 108, 2)" v-if="operationState.isWanted">
          已想读
        </n-button>


        <n-button secondary strong :render-icon="renderIcon2" class="operation-btn" color="rgb(2, 108, 2)" v-if="!operationState.isFavourited">
          添加到收藏夹
        </n-button>
      ...
      <div class="comment-container">
        <div class="container">
        <CommentCard v-for="comment in comments" :key="comment.id" class="card" :book-info="book"></CommentCard>
      </div>
      </div>
  ...
  >
    ...
    <div>
      <n-input type="textarea" maxlength="250" placeholder="说说你读过之后的感受吧" v-model:value="rateObj.comment" resize="none"
        :autosize="{
          minRows: 3,
          maxRows: 5
        }" show-count 
