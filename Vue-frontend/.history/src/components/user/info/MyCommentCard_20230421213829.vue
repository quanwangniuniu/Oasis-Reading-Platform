    ...
    <div class="container">
      ...
      <div class="comment-container">
        <div class="comment-info">
          <div class="title">
            <router-link v-if="userType == 2" :to="'/admin/books/' + commentObj.book_id" class="name">{{ commentObj.title }}</router-link>
                <router-link v-if="userType != 2" :to="'/books/' + commentObj.book_id" class="name">{{ commentObj.title }}</router-link>
          </div>
          <div class="rating">
            <n-rate allow-half readonly :default-value=commentObj.rate/2 size="small" />
          </div>
          <div class="date">
            {{ created_time }}
          </div>
