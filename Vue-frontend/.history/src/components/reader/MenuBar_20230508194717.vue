...
import { PlayForwardSharp, MenuOutline, TextSharp, SunnyOutline } from '@vicons/ionicons5'
import Content from './Content.vue'

const props = defineProps(['ifTitleAndMenuShow', 'themeList', 'bookAvailable', 'navigation'])
...
const emit = defineEmits(['setFontSize', 'setTheme', 'onProgressChange', 'jumpTo']);
const setFontSize = () => {
    emit('setFontSize', 12 + (fontsize.value / 25) * 4);
}
...
const showSetting = (tag) => {
    if (!isSettingShow.value) {
        if (tag === 3) {
            ...
            console.log(ifShowContent.value)
        } else {
...
const updateProgress = () => {
    emit('onProgressChange', progress.value)
...
<template>
    <div class="menu-bar">
        ...
        <Content :ifShowContent="ifShowContent" v-show="ifShowContent" :navigation="navigation"
            :bookAvailable="bookAvailable" @jumpTo="jumpTo"></Content>
