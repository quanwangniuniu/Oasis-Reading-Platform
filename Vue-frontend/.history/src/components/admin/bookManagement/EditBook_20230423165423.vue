...
const updateBook = () => {
  bookItem.value.series = bookItem.value.serie
  bookItem.value.series = ''
  console.log(bookItem.value.series + '---------------')
  console.log(bookItem.value.publish_year + '---------------')
  api_updateBook(bookItem.value).then(res => {
    console.log(res.data)
    if(res.data.code == 200){
      open()
      router.push( {path: '/admin/managebooks/list'})
    }
  })
}
...
const CommitButtonClick = () => {
  formRef.value?.validate((errors: any) => {
    if(bookItem.value.url == undefined || bookItem.value.url == ''){
      message.error('请先上传书籍封面')
    }
    if (!errors) {
      updateBook()
    }
...
<template>
    <div class="container">
        ...
        <div class="img-upload-and-submit">
          ...
          <div class="submit">
            <button @click="CommitButtonClick">修改</button>
          </div>
        </div>
    </div>
