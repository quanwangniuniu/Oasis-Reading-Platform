...
import { api_getTotalPages, api_deleteBookById, api_getBooksByPage } from '../../axios/api'
...
const editBook = (bookItem) => {
  ...
  router.push({ path: '/admin/managebooks/edit', query: { id: bookItem.id } })
}

//删除图书函数, 
const deleteBook = (row) => {
  api_deleteBookById(row.id).then(res => {
    getBooks(current_page.value)
    ElMessage({
      message: '删除成功',
      type: 'success',
      duration: 1000
...
<template>
  <div class="list-container">
    <el-table :data="books" style="width: 100%;" class="book-list">
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template #default="scope">
          <el-button  type="success" size="small" @click="editBook(scope.row)">编辑</el-button>
    ...
    <div class="pagination">
      <n-pagination v-model:page="current_page" :page-count="totalPages" />
    </div>
  </div>
</template>
